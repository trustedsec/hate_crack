...
CONFIG_SCHEMA: tuple[ConfigKey, ...] = (
    ...
    # Seconds the whole Hybrid attack may spend, shared across all of its
    # passes -- sixteen for a single wordlist (?s?d and then ?a, lengths 1-4,
    # appended and prepended). Time-bounded rather than length-bounded because
    # the cost is the wordlist times the mask: over the default rockyou.txt the
    # ?a passes alone are ~1.18e15 candidates, roughly ten hours for NTLM on
    # hardware doing 32 GH/s and hopeless on a slow hash, while over a small
    # targeted list every pass finishes outright. One hour spends a bounded
    # amount either way. 0 means no limit -- every pass runs to exhaustion,
    # which is a deliberate choice to make rather than a default to inherit.
    ConfigKey("HCAT_HYBRID_MAX_RUNTIME", "hcatHybridMaxRuntime", "int", 3600),

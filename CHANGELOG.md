...
### Added
- **The Spoonman Attack (menu 22) now says how many basewords the optimized kernel will throw away.** hashcat's `-O` caps candidate length and drops anything over it in silence — verified against hashcat v7.1.2 in mode 0, a 31-character plaintext cracks, a 32-character one does not, and hashcat prints no warning and exits 0. Spoonman's baseword list is unusually exposed to this because it holds whole literal passwords (every letterless or unrepresentable corpus entry becomes its own baseword), so a corpus with long entries lost them with nothing to show for it. The attack now counts the over-cap basewords before starting hashcat and prints one line naming the count and the flag that would keep them (`--no-optimized-kernel` / `--no-optimize`). Nothing else changes: it does not prompt, does not filter candidates, and does not alter what hashcat is asked to run. The cap is a module-level constant (`OPTIMIZED_KERNEL_MAX_PLAIN_LENGTH`) holding the mode-0 figure — the real limit is mode-dependent, so the warning is a heuristic and no attempt is made to derive it per hash mode. The count streams the baseword file a line at a time, since it can be nearly as large as the corpus, and an unreadable file skips the check rather than failing the attack.

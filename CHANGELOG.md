...
### Changed
- **The Spoonman Attack (menu 9) generates more compact rules via cost-optimized case encoding.** The baseword derivation previously emitted one `T{p}` (toggle) per uppercase letter, regardless of context, so a mostly-uppercase password like `SUPERword` generated a 9-function rule (`T1T2T3T4T5T6T7T8`) where `uT8` (uppercase all, then toggle position 8) costs 2 functions. The derivation now compares four candidate case encodings against the all-lowercase baseword — none (cost 0), direct (one toggle per uppercase), u+invert (uppercase all, then toggle each lowercase), and c+fix (capitalize, then toggle uppercase at positions > 0 and position 0 if it lands lowercase) — and picks the minimum cost, breaking ties in order: none, c, u, direct. Positions beyond 35 (unaddressable in hashcat's 36-character position alphabet) disqualify only that candidate, not the whole password. This lifts some passwords out of the `MAX_RULE_FUNCTIONS` fallback to literal (password as its own baseword) and improves rule reusability across passwords with different case patterns, raising coverage for no cost in rule file size.

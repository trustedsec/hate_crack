...
### Changed
- **Downloading a newer Hashmob found corpus now offers to remove the older same-tier copies it supersedes.** The official wordlist listing only ever publishes the newest dated release (`hashmob.net_<date>[.<tier>].found.7z`), and each release is additive over the previous one, so keeping the old file wastes space for candidates that are all present in the new one. After a file is downloaded and extracted -- or found already present -- the download flow scans the wordlists directory for older files of the same tier (a `tiny` list never touches `huge`, and the tier is matched as a whole token so `large` never matches `larger`) and asks once per file before deleting it. Nothing is removed until the newest copy is on disk, so a failed download or extraction leaves the old file in place, and a non-interactive context declines rather than deleting.

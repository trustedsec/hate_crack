...
### Added
- **The Hybrid attack now finishes with a full-charset `?a` sweep, lengths 1-4 in both directions, bounded by a configurable time budget (`hcatHybridIncrementRuntime`, seconds, default 3600).** The existing passes only ever tried `?s?d`, so a decoration containing a letter was out of reach. `?a` is every printable character, making the sweep a strict superset of those passes: 95**4 rather than 43**4 at the longest mask, about 1.2e15 candidates over rockyou.txt — roughly ten hours for NTLM on hardware doing 32 GH/s and hopeless on a slow hash, which is why it is time-bounded rather than length-bounded and runs after the cheap passes have had their chance. `0` disables it, silently; the fixed-length passes are never time-capped.

  Three implementation details are load-bearing, and two of them come from hashcat behaviour verified against 7.1.2 rather than from any documentation. **The sweep is one hashcat invocation per length, not a single `--increment 1-4`.** The candidates are identical either way — `--increment` queues each length as its own job, visible as `Guess.Queue.Mod: 2/6` — but `--runtime` is applied *per queued length*, so one `--increment` pass under a one-hour cap can spend four hours; and a length cut short inside an `--increment` run still exits **1**, the code `_run_hcat_cmd_uncovered` reads as "keyspace exhausted", so the coverage store would have recorded ground that was never enumerated. A standalone mask cut short by `--runtime` exits **4** instead, which is not recorded — so each length that genuinely completes is banked and one that runs out of budget is retried next time. **The passes share one deadline**, each receiving what remains of it, so the budget governs the sweep rather than each pass individually. **Ordering is by mask length across all wordlists**, not per wordlist, because with a shared budget the length-4 pass over the first wordlist would otherwise consume the whole hour and leave a second wordlist without even its length-1 pass. A sweep that runs out of budget says how many passes it did not run and which setting to raise, rather than ending quietly.

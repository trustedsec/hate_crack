...
# CHANGELOG.md does not count: it changes on nearly every commit in this repo,
# so treating it as "docs updated" silenced this audit on 53 of the last 100
# code commits. An entry records that something changed; it does not document
# how the thing behaves.
HAS_DOC_CHANGES=false
if echo "$CHANGED_FILES" | grep -vE '(^|/)CHANGELOG\.md$' | grep -qE "README|CLAUDE|\.md$"; then
  HAS_DOC_CHANGES=true
  echo -e "${YELLOW}[Documentation Audit]${NC} Documentation files detected"
fi

# Check if code files changed (Python, config, etc.).
# tests/ is excluded: a test-only commit changes no documented behaviour, so
# demanding a README edit for one is pure noise.
HAS_CODE_CHANGES=false
if echo "$CHANGED_FILES" | grep -vE '^tests/' | grep -qE "\.py$|config\.json|pyproject\.toml|Makefile"; then
...
if [ "$HAS_CODE_CHANGES" = true ] && [ "$HAS_DOC_CHANGES" = false ]; then
  ...
  # shellcheck disable=SC2001
  echo "$CHANGED_FILES" | grep -vE '^tests/' | grep -E "\.py$|config\.json|pyproject\.toml|Makefile" | sed 's/^/  /'
  echo ""

...
for commit in $COMMITS; do
  ...
  # Check for code changes. Keep this pattern identical to audit-docs.sh,
  # including the tests/ exclusion documented there.
  CODE_CHANGED=$(echo "$CHANGED" | grep -vE '^tests/' | grep -E "\.py$|config\.json|pyproject\.toml|Makefile" || true)

  # Check for doc changes. Keep this pattern identical to audit-docs.sh, including
  # the CHANGELOG.md exclusion documented there.
  DOC_CHANGED=$(echo "$CHANGED" | grep -vE '(^|/)CHANGELOG\.md$' | grep -E "README|CLAUDE|\.md$" || true)

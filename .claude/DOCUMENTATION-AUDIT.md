...
- **Code files**: `*.py`, `pyproject.toml`, `Makefile`, `config.json` —
  **except anything under `tests/`.** A test-only commit changes no documented
  behaviour, so demanding a README edit for one is pure noise.
- **Documentation files**: `README.md`, `*.md` in any directory, `CLAUDE.md` —
  **except `CHANGELOG.md`.** It changes on nearly every commit here, so
  counting it as a documentation update silenced the audit almost entirely. A
  changelog entry records that something changed; it does not document how the
  thing behaves.

The script will warn if code changes don't have corresponding documentation updates.

Both exclusions were added on 2026-08-21 after measuring the last 100 commits:
the audit flagged **23** commits before, **61** with `CHANGELOG.md` excluded,
and **52** once `tests/` was excluded too. The 23 was the misleading number —
a changelog entry was silently satisfying the check on most real changes.

`audit-docs.sh` and `check-docs.sh` must keep both patterns identical — they
each carry their own copy, so a change to one needs the same change to the
other. `tests/test_doc_audit_parity.py` enforces that: it runs every case
against both scripts, using real commits in throwaway repos, and fails if the
two disagree.

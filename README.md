...
Version 2.10.7
  - Bumped the pinned dev/test dependency `pytest` from 9.0.2 to 9.0.3 to clear the vulnerable tmpdir-handling advisory (GHSA-6w46-j5rx-g56g, affects pytest < 9.0.3). This is the development-scope test runner only — no runtime dependency change. (The same advisory was previously addressed in the `HashcatRosetta` submodule in 2.10.1; this closes it out for hate_crack's own `pyproject.toml`/`uv.lock` pin.)
